...
// Ganti fungsi updateUI yang lama dengan ini
...
// KODE BARU (Logout dengan Konfirmasi Cantik)
    ...
    $('#reservationForm').on('submit', async function(e) {
        ...
        const formData = {
            customer_name: $('#name').val(), phone_number: $('#phone').val(),
            reservation_date: $('#date').val(), reservation_time: $('#time').val(),
            num_guests: $('#guests').val(), special_request: $('#special_request').val()
        };

        if (!formData.customer_name || !formData.reservation_date || !formData.reservation_time || !formData.num_guests) {
            alert('Please fill all required fields (Name, Date, Time, Number of Guests).');
            return;
        }
